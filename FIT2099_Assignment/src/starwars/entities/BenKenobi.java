			...
			scheduler.schedule(attack.affordance, ben, 1);
			 ...
			 // if it is equal to one, the only thing here is this Player, so there is nothing to report
				...
				for (SWEntityInterface entity : contents) {
					...
					if (entity.getSymbol() == "o"  && ben.getHitpoints() < 1000 && itemCarried != null && entity.getHitpoints()> 0) { // don't include self in scene description
						this.addAffordance(new Leave(this, messageRenderer));
						say(getShortDescription() + "is standing to drink from the canteen");
						
					}
					else if (entity.getSymbol() == "o"  && ben.getHitpoints() < 1000 && itemCarried == null && entity.getHitpoints()> 0) { // don't include self in scene description
						this.addAffordance(new Healing(this, messageRenderer));
						say(getShortDescription() + "is drinking from the canteen");
				}
					else if (entity.getSymbol() == "@" && ((SWActor) entity).getForce() <1000 ) { // don't include self in scene description
